...
export function AdvancedAbsenceCalendar() {
  ...
  const { reportAbsence, getStudentAbsences, loading } = useAttendance();
  ...
  const loadScheduledAbsences = async (studentId: string) => {
    if (!studentId) return;
    setLoadingAbsences(true);
    setFetchError(null);
    try {
      const data = await getStudentAbsences(parseInt(studentId));
      setScheduledAbsences(data);
    } catch (err: any) {
      setFetchError(err.message || 'Error loading absences');
    } finally {
      setLoadingAbsences(false);
    }
  };

  // Refetch when student changes
  useEffect(() => {
    loadScheduledAbsences(selectedStudent);
  }, [selectedStudent]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await loadScheduledAbsences(selectedStudent);
      setSuccess(true);
      setSelectedDates([]);
      setReason('');
    ...
    <Card title="Schedule Future Absences" className="relative overflow-hidden">
      ...
      {success ? (
        ...
        >
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-2">Absence Scheduled!</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {loadingAbsences && <p className="text-sm text-gray-500 mb-2">Loading scheduled absences...</p>}
            {fetchError && <p className="text-sm text-red-500 mb-2">Error: {fetchError}</p>}
            <div className="grid grid-cols-7 gap-2">
              {dates.map((dateInfo) => {
                const isScheduled = scheduledAbsences.some((a) => a.date === dateInfo.date);
                ...
                return (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() => !isScheduled && handleDateToggle(dateInfo.date)}
                    disabled={isScheduled || dateInfo.isWeekend}
                    className={`
                      relative p-3 rounded-xl text-center transition-all duration-200
                      ${isSelected
                        ? 'bg-gradient-to-br from-primary-300 to-primary-500 text-white shadow-lg shadow-primary-500/30'
                        : isScheduled
                        ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                        : dateInfo.isWeekend
                        ? 'bg-gray-50 text-gray-300 cursor-not-allowed'
                        : 'bg-white border-2 border-gray-200 text-gray-700 hover:border-primary-300 hover:bg-primary-100'
                      }
                    `}
                  >
                    <div className="text-xs font-medium opacity-75">{dateInfo.dayName}</div>
          ...
          {selectedDates.length > 0 && (
            ...
            >
              ...
              <div className="bg-primary-50 border border-primary-200 rounded-xl p-4">
                <h4 className="text-sm font-semibold text-primary-900 mb-2">Selected Dates:</h4>
              ...
              <Button type="submit" isLoading={isSubmitting} className="w-full" size="lg">
                Schedule Absence ({selectedDates.length} {selectedDates.length === 1 ? 'day' : 'days'})
              </Button>
            </motion.div>
          )}
        </form>
